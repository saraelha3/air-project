...
function buildPlumeLayers(windDeg, windSpeed, aqi) {
  ...
  const ZONES = 7, STEPS = 64, layers = [];
  for (let z = ZONES; z >= 1; z--) {
    const t = z / ZONES, dist = reach * t, width = spread * t * (1 + t * 0.5);
    const coords = [];
    for (let i = 0; i <= STEPS; i++) {
      const frac = i/STEPS, d = dist*(1-frac*0.55), w = width*(1-frac*0.3);
      const cx = OCP_CENTER[1]+d*Math.sin(angleRad), cy = OCP_CENTER[0]+d*Math.cos(angleRad)*0.75;
      const px = -Math.cos(angleRad)*w*0.5, py = Math.sin(angleRad)*w*0.5*0.75;
    ...
    const [color, opacity] = conc>0.80?["#dc2626",0.38]:conc>0.62?["#f97316",0.30]:conc>0.46?["#eab308",0.26]:conc>0.30?["#22c55e",0.22]:conc>0.16?["#38bdf8",0.18]:["#3b82f6",0.13];
...
function buildWindArrows(windDeg) {
  const angleRad = ((windDeg+180)%360)*Math.PI/180;
  return [[-0.18,-0.12],[-0.18,0],[-0.18,0.12],[-0.08,-0.06],[-0.08,0.06],[0.04,-0.10],[0.04,0.04]].map(([dy,dx]) => {
    const lat0 = OCP_CENTER[0]+dy, lon0 = OCP_CENTER[1]+dx;
    return [[lat0,lon0],[lat0+0.025*Math.cos(angleRad),lon0+0.025*Math.sin(angleRad)]];
  });
}
...
export default function RadarMap({ weather }) {
  ...
  const layersRef  = useRef({ plume:[], arrows:[], perimeter:null, sources:[], ocpMarker:null });
  ...
  const changeTheme = theme => {
    ...
    tileRef.current = L.tileLayer(MAP_THEMES[theme].url, { maxZoom:18, subdomains:"abcd" }).addTo(map);
  };
  ...
  const renderLayers = key => {
    ...
    if (refs.ocpMarker) map.removeLayer(refs.ocpMarker);
    ...
    refs.perimeter = L.polygon(OCP_PERIMETER, { color:"#4ade80", weight:2.5, fillColor:"#4ade80", fillOpacity:0.06, dashArray:"8 5" }).addTo(map);
    refs.perimeter.bindPopup(`<div style="font-family:sans-serif"><strong style="color:#4ade80">⬛ Périmètre OCP Safi</strong><br/><small>📍 32.231°N, 9.251°O</small></div>`);

    buildWindArrows(data.windDeg).forEach(pts => {
      const line = L.polyline(pts, { color:"rgba(255,255,255,0.5)", weight:1.5 }).addTo(map);
      const [p0,p1] = pts, dy=p1[0]-p0[0], dx=p1[1]-p0[1], len=Math.sqrt(dy*dy+dx*dx)||1, sz=0.007;
    ...
    refs.ocpMarker = L.marker(OCP_CENTER, { icon:ocpIcon }).addTo(map);
    refs.ocpMarker.bindPopup(`<div style="font-family:sans-serif"><strong style="color:#4ade80">🏗 Usine OCP Safi</strong><br/><small>📍 32.2310°N, 9.2508°O</small></div>`);
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      setLiveData(prev => ({
        ...prev,
        pm25: +(Math.max(20, Math.min(80,  prev.pm25+(Math.random()-.5)*4))).toFixed(1),
        pm10: +(Math.max(40, Math.min(130, prev.pm10+(Math.random()-.5)*5))).toFixed(1),
        so2:  +(Math.max(50, Math.min(150, prev.so2 +(Math.random()-.5)*6))).toFixed(1),
        aqi:  Math.round(Math.max(30, Math.min(180, prev.aqi+(Math.random()-.5)*5))),
      }));
    }, 3500);
  ...
  const panelStyle = { position:"absolute", background:"rgba(8,20,36,0.88)", border:"0.5px solid rgba(255,255,255,0.13)", backdropFilter:"blur(10px)", borderRadius:12, color:"#e2e8f0", zIndex:1000 };
  const tabs = [{key:"now",label:"Actuel"},{key:"h6",label:"+6h"},{key:"h12",label:"+12h"},{key:"h24",label:"+24h"}];

  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", overflow:"hidden", borderRadius:"var(--radius)", position:"relative", background:"#0a1628" }}>
      {/* Header */}
      <div style={{ padding:".65rem 1rem", borderBottom:"1px solid rgba(255,255,255,0.08)", display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
        <h3 style={{ fontSize:".72rem", fontWeight:700, textTransform:"uppercase", letterSpacing:".1em", color:"#94a3b8", display:"flex", alignItems:"center", gap:6, margin:0 }}>
      ...
      {/* Thème */}
      <div style={{ padding:"6px 10px", borderBottom:"0.5px solid rgba(255,255,255,0.07)", display:"flex", gap:5, alignItems:"center", flexShrink:0 }}>
        <span style={{ fontSize:10, color:"#64748b", fontWeight:600, textTransform:"uppercase", letterSpacing:".05em" }}>Carte :</span>
        {Object.entries(MAP_THEMES).map(([k,t]) => (
          <button key={k} onClick={() => changeTheme(k)} style={{ padding:"4px 11px", fontSize:10.5, borderRadius:6, border:`1.5px solid ${mapTheme===k?"#4ade80":"rgba(255,255,255,0.15)"}`, background:mapTheme===k?"rgba(74,222,128,0.15)":"transparent", color:mapTheme===k?"#4ade80":"#94a3b8", cursor:"pointer", fontWeight:mapTheme===k?700:400, transition:"all .18s" }}>{t.name}</button>
        ))}
      </div>
      ...
      <div style={{ position:"relative", flex:1, minHeight:0 }}>
        ...
        <div style={{ ...panelStyle, top:10, left:10, display:"flex", gap:5, padding:7 }}>
          {tabs.map(t => (
            <button key={t.key} onClick={() => handleForecast(t.key)} style={{ fontSize:11.5, padding:"6px 13px", borderRadius:8, cursor:"pointer", fontWeight:activeFc===t.key?700:500, border:`2px solid ${activeFc===t.key?"#4ade80":"rgba(255,255,255,0.22)"}`, background:activeFc===t.key?"rgba(74,222,128,0.2)":"rgba(255,255,255,0.04)", color:activeFc===t.key?"#4ade80":"#cbd5e1", transition:"all .18s" }}>{t.label}</button>
          ))}
        </div>

        {/* Métriques */}
        <div style={{ ...panelStyle, top:10, right:10, width:178, padding:"11px 13px" }}>
          <div style={{ fontSize:10, fontWeight:700, color:"#4ade80", letterSpacing:1.2, textTransform:"uppercase", marginBottom:9, display:"flex", alignItems:"center", gap:5 }}>
          ...
          {[["🌡","Temp.",`${liveData.temp}°C`],["💧","Humidité",`${liveData.hum}%`],["💨","Vent",`${liveData.windSpeed} m/s`],["🧭","Direction",`${liveData.windDeg}°`],["☁","PM2.5",`${typeof liveData.pm25==="number"?liveData.pm25.toFixed(1):liveData.pm25} µg/m³`],["🌫","PM10",`${typeof liveData.pm10==="number"?liveData.pm10.toFixed(1):liveData.pm10} µg/m³`],["⚗","SO₂",`${typeof liveData.so2==="number"?liveData.so2.toFixed(1):liveData.so2} µg/m³`],["🔬","NOx",`${liveData.nox} µg/m³`]].map(([ico,label,val]) => (
          ...
          <div style={{ marginTop:8 }}>
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, color:"#94a3b8", marginBottom:3 }}>
              <span>IQA</span><span style={{ color:aqiLevel.color, fontWeight:700 }}>{aqiLevel.label} ({liveData.aqi})</span>
            </div>
        ...
        <div style={{ ...panelStyle, bottom:10, right:10, padding:"8px 12px", minWidth:130 }}>
          ...
          {[["#dc2626","Très élevée"],["#f97316","Élevée"],["#eab308","Modérée"],["#22c55e","Faible"],["#38bdf8","Très faible"],["#3b82f6","Minimale"]].map(([c,l]) => (
            <div key={l} style={{ display:"flex", alignItems:"center", gap:6, fontSize:10, color:"#cbd5e1", marginBottom:3 }}>
              <div style={{ width:26, height:6, borderRadius:3, background:c }}/>{l}
            </div>
          ))}
        </div>
      </div>
