...
export default function HourlyChart({ forecast }) {
  ...
  const data = forecast.slice(0,8).map(item=>({
    ...
    temp:Math.round(item.temperature), humidity:item.humidity,
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div style={{background:"rgba(9,21,16,.97)",border:"1px solid rgba(174,195,176,.18)",borderRadius:10,padding:"10px 14px",fontSize:".8rem",boxShadow:"0 8px 24px rgba(0,0,0,.4)"}}>
        ...
        {payload.map((p,i)=>(<p key={i} style={{color:p.color,marginBottom:2}}>{p.name}: <strong>{p.value}{p.name==="Temp"?"°C":p.name==="Humidité"?"%":" m/s"}</strong></p>))}
      </div>
    );
  };
  return (
    <div className="chart-container animate-in" style={{padding:"1.1rem 1.2rem"}}>
      <h3 style={{fontSize:".72rem",fontWeight:700,textTransform:"uppercase",letterSpacing:".1em",color:"var(--text-muted)",marginBottom:"1rem"}}>📈 Prévisions horaires (24h)</h3>
