...
function genHistory() {
  ...
  for (let i=0; i<40; i++) {
    ...
    rows.push({
      id:i, date:d.toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"}),
...
export default function History() {
  ...
  const handleExport = () => {
    ...
    const rows=ALL.map(r=>`${r.date},${r.time},${r.label},${r.confidence}%,${r.gasFlow},${r.windDir},${r.windSpeed} m/s,${r.temperature}°C,${r.humidity}%,${r.probabilities["Pas de risque"]},${r.probabilities["Risque faible"]},${r.probabilities["Risque moyen"]},${r.probabilities["Risque élevé"]}`).join("\n");
            ...
            <tbody>
              {filtered.map(row=>(
                <React.Fragment key={row.id}>
                  ...
                  {expanded===row.id && (
                    <tr>
                      <td colSpan={9} style={{ background:"rgba(107,144,113,.04)", padding:"1rem 1.2rem" }}>
